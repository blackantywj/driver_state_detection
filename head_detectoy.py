  ...
  #      print('pitch:{}, yaw:{}, roll:{}'.format(pitch, yaw, roll))
    ...
    def get_pose_estimation_in_euler_angle(self, landmark_shape, im_szie):
        try:
            ...
            ret, rotation_vector, translation_vector, camera_matrix, dist_coeffs = self.get_pose_estimation(im_szie, image_points)
            if ret != True:
...
#        print("used_time:{} sec".format(round(used_time, 3)))
        ...
        ret, pitch, yaw, roll = self.get_euler_angle(rotation_vector)
        euler_angle_str = [pitch, yaw, roll]
        #print(euler_angle_str)
#        client_send.sendall(bytes(euler_angle_str, encoding="utf-8"))
...
#        cv2.putText( im, euler_angle_str, (0, 120), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1 )
        cv2.imwrite("capture_add.jpg", im)
        return euler_angle_str
#        cv2.imshow("Output", im)
#        cv2.
#waitKey(1)
head_detect = head_detector()
